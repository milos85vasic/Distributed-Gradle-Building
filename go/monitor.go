...
// Prometheus metrics for monitor
var (
	monitorWorkersTotal = prometheus.NewGauge(
		prometheus.GaugeOpts{
			Name: "monitor_workers_total",
			Help: "Total number of registered workers",
		},
	)
	monitorBuildsTotal = prometheus.NewGauge(
		prometheus.GaugeOpts{
			Name: "monitor_builds_total",
			Help: "Total number of builds tracked",
		},
	)
	monitorAlertsTotal = prometheus.NewCounterVec(
	...
	monitorHTTPRequestsTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
...
func (m *Monitor) RegisterWorker(id string, host string, status string) {
	...
	m.workers[id] = worker
	monitorWorkersTotal.Set(float64(len(m.workers)))
}
